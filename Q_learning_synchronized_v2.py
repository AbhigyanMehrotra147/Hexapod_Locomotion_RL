import my_hexapod_synchronized_legs_env_v2 as rvd
import numpy as np
...
Hexapod = p.loadURDF("Hexapod/Hexapod.urdf", basePosition = [0, 0, 0.2])
...
for episode in range(episodes):
    ...
    Hexapod = p.loadURDF("Hexapod/Hexapod.urdf", basePosition=rvd.INIT_POSITION)
    Plane = p.loadURDF("Plane.urdf")
...
plt.title("total_reward vs Episodes synchronized_Q_learning v2")
plt.xlabel("Num Episodes")
plt.ylabel("Total Reward")
plt.show()
plt.savefig("total_reward vs Episodes synchronized_Q_learning v2.png")

plt.plot(num_steps_walked_per_episode)
plt.title("steps vs Episodes synchronized Q learnin v2.")
plt.xlabel("Num Episodes")
plt.ylabel("Steps")
plt.show()
plt.savefig("steps vs Episodes synchronized Q learning v2.png")


np.save("Q_table_synchronized_v2.npy", Q_table)



def simulation():
    # === Simulation of Trained Policy ===
    print("Starting simulation with trained policy...")

    # Reinitialize the simulation in GUI mode
    p.disconnect()
    ...
    while not done and count < 1000:
        ...
        position, _ = p.getBasePositionAndOrientation(Hexapod)
        done = bool(position[2] < 0.08)  # Stop if the robot falls
        count += 1
